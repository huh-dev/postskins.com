...
const { items, groupedItems, totalCount, status, stale, isLoading, errorMessage, load } = useInventory()
...
<template>
  <div class="mx-auto min-h-svh w-full max-w-6xl px-2 py-4 sm:px-3 sm:py-6">
    ...
    <template v-else>
      <header class="surface-panel mb-3 flex items-center justify-between gap-3 rounded-lg p-2 sm:p-2.5">
        <div class="min-w-0 px-1">
          <h1 class="text-sm font-semibold text-foreground">Inventory</h1>
          <p class="text-xs text-muted-foreground">
            <template v-if="status === 'success'">
              {{ totalCount }} {{ totalCount === 1 ? 'item' : 'items' }}
              <span v-if="groupedItems.length !== totalCount" class="text-muted-foreground/80">
                · {{ groupedItems.length }} stacks
              </span>
              <span v-if="stale" class="text-amber-500"> · showing last known (Steam unavailable)</span>
            </template>
        ...
        <button
          type="button"
          class="flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-border/60 bg-sidebar-accent/60 px-2.5 text-xs text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-foreground disabled:pointer-events-none disabled:opacity-50"
          :disabled="isLoading"
          @click="load({ fresh: true })"
        >
          <RiLoopRightLine class="size-3.5" :class="{ 'animate-spin': isLoading }" />
          Refresh
        </button>
      </header>
      ...
      <div v-else-if="isLoading && items.length === 0" class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        <div v-for="n in 18" :key="n" class="surface-panel animate-pulse overflow-hidden rounded-lg">
